...
export const menuItems = [
  ...
  { name: "Graphics Processing Units", image: GPU1, category: "gpu", products: [
    ...
    { 
      name: "12GB RX7700XT ASROCK", 
      price: "PHP 27,995",
      details: "A high-end GPU with excellent cooling solutions for overclocking and sustained performance.",
      specifications: "12GB GDDR6 | 192-bit | 2599 MHz Boost Clock | PCIe 4.0 | 2x HDMI, 2x DP"
    }
  ], 
  ...
  { name: "Random Access Memory", image: Ram, category: "ram", products: [
    ...
    {
      name: "32GB T-FORCE DELTA RGB Kit 6400MHz White",
      price: "PHP 7,699",
      details: "An ultra-fast DDR5 RAM kit with a sleek white design and dynamic RGB.",
      specifications: "32GB (2x16GB) DDR5 | 6400MHz | CL32 | 1.4V | XMP 3.0 | RGB Lighting"
    }
  ],
...
const handleStartOver = () => {
  ...
  useEffect(() => {
    if (menuItems.length > 0 && selectedItem >= 0 && selectedItem < menuItems.length) {
      ...
      if (selectedCategory) {
        console.log("Fetching products for category:", selectedCategory); // Debugging log
  
        const fetchedProducts = generateSampleProducts(selectedCategory); // ✅ Make sure this function is correct
        ...
        setProducts([]); // Fallback for missing category
      }
    }
  }, [selectedItem]);
  ...
  useEffect(() => {
    ...
    const allProducts = menuItems.flatMap((category) =>
      ...
      setRandomHotPicks(shuffledProducts.slice(0, 9)); // Pick first 9 for Hot Picks
      setRandomValueForMoney(shuffledProducts.slice(9, 15)); // Pick next 6 for Value for Money
    }, []);
  ...
  return (
        ...
        <div className="menu">
          ...
          <div className="main-content">
            ...
            <section className="home-section">
              <h3>WHAT’S NEW!</h3>
              <div className="home-large-box" onClick={() => handleLargeBoxClick("whats-new")}>
            ...
            <section className="home-section">
              <h3>On Sale🏷️</h3>
              <div className="home-large-box" onClick={() => handleLargeBoxClick("on-sale")}>
                Click to See On Sale Items!
              </div>
            </section>
          </div>
  ) : (
    ...
    <>
        ...
        <div className="brand-filter-container">
          ...
          <div className="grid">
            {products && products.length > 0 ? ( // ✅ Check if products exist before mapping
              products.map((product, idx) => (
                <button 
                  key={idx} 
                  className="grid-item" 
                  onClick={() => handleProductClick(menuItems[selectedItem]?.category || "unknown", product, idx)} 
                  aria-label={`View details for ${product.name}`}
                >
                  <div className="dot"></div>
                  <div>{product.name}</div>
                  <div className="price">
                  {product.price ? `₱${Number(product.price).toLocaleString()}` : "Price Unavailable"}
                  </div>
                </button>
              ))
              ) : (
                <p>No products available.</p> // ✅ Display message if products are empty
              )}
            </div>
          </div>
      </>
